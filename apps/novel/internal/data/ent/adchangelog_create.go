...
func (aclc *AdChangeLogCreate) check() error {
	if _, ok := aclc.mutation.UserId(); !ok {
		return &ValidationError{Name: "userId", err: errors.New(`ent: missing required field "AdChangeLog.userId"`)}
	}
	if _, ok := aclc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "createdAt", err: errors.New(`ent: missing required field "AdChangeLog.createdAt"`)}
	}
	if _, ok := aclc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updatedAt", err: errors.New(`ent: missing required field "AdChangeLog.updatedAt"`)}
	}
	if _, ok := aclc.mutation.CreateBy(); !ok {
		return &ValidationError{Name: "createBy", err: errors.New(`ent: missing required field "AdChangeLog.createBy"`)}
	}
	if _, ok := aclc.mutation.UpdateBy(); !ok {
		return &ValidationError{Name: "updateBy", err: errors.New(`ent: missing required field "AdChangeLog.updateBy"`)}
	}
	if _, ok := aclc.mutation.TenantId(); !ok {
		return &ValidationError{Name: "tenantId", err: errors.New(`ent: missing required field "AdChangeLog.tenantId"`)}
	}
	if _, ok := aclc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`ent: missing required edge "AdChangeLog.user"`)}
	}
	return nil
}
