...
func genConvert(projectPath, model string, sc load.Schema) bool {
	bf := str.NewBuffer()
	bf.Append(`// Code generated by Caisin. DO NOT EDIT.
...
`
	...
	for i, f := range sc.Fields {
		...
		reqContent.Append(fmt.Sprintf("\t\t%s:       v.%s,", lu, lu))
		...
		bf.Append(tab).Append("//").Append(f.Comment).Append("\n")
		//字段
		bf.Append(tab).Append(parseType(f.Info.Type)).Append(space).Append(fieldName).
			Append(space).Append("=").Append(space).Append(i + 1).Append(";\n")
	}
	bf.Append(fmt.Sprintf(funTmp, name+"Req2Data", "v1."+name+"Req", "ent."+name, "ent."+name, reqContent.String()))
	buffer := bf.Buffer
	fileName := fmt.Sprintf("%s/apps/%s/internal/convert/%s.go", projectPath, model, str.Camel2Case(name))
	file.MakeDir(path.Dir(fileName))
