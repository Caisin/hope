...
func (aclq *AssetChangeLogQuery) sqlAll(ctx context.Context) ([]*AssetChangeLog, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, assetchangelog.ForeignKeys...)
	}
	...
	if query := aclq.withUser; query != nil {
		...
		for i := range nodes {
			if nodes[i].social_user_asset_logs == nil {
				continue
			}
			fk := *nodes[i].social_user_asset_logs
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "social_user_asset_logs" returned %v`, n.ID)
			}
			for i := range nodes {
