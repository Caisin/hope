...
func (tx *Tx) init() {
	tx.NovelPayConfig = NewNovelPayConfigClient(tx.config)
	tx.NovelTag = NewNovelTagClient(tx.config)
	tx.PageConfig = NewPageConfigClient(tx.config)
	tx.QiniuConfig = NewQiniuConfigClient(tx.config)
	tx.ResourceGroup = NewResourceGroupClient(tx.config)
	tx.ResourceStorage = NewResourceStorageClient(tx.config)
	tx.ScoreProduct = NewScoreProductClient(tx.config)
	tx.Task = NewTaskClient(tx.config)
	tx.UserAnalysisStatistics = NewUserAnalysisStatisticsClient(tx.config)
	tx.UserConsume = NewUserConsumeClient(tx.config)
	tx.UserResource = NewUserResourceClient(tx.config)
	tx.UserResourceRecord = NewUserResourceRecordClient(tx.config)
	tx.VipType = NewVipTypeClient(tx.config)
}
...
// applies a query, for example: NovelPayConfig.QueryXXX(), the query will be executed
